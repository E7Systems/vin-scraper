  ...
  def scrape
    YEARS.each do |year|
      @agent.get("#{ROOT_URL}/#{year}") do |makes_page|
        makes_page.search('div.mainframe table a').each do |make_anchor|
